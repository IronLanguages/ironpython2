...
class GeneratorTest(IronPythonTestCase):
    ...
    def test_generator_expressions(self):
        ...
        self.assertEqual(list((i,j) for i in xrange(2) for j in xrange(3)), [(0, 0), (0, 1), (0, 2), (1, 0), (1, 1), (1, 2)])
        self.assertEqual(list((i,j) for i in xrange(2) for j in xrange(i+1)), [(0, 0), (1, 0), (1, 1)])
    ...
    def test_nested_generators(self):
        ...
        for a,b in outergen():
            ...
            self.assertEqual(range(a), list(b))
        
        
        def f():
            yield "Import inside generator"
        
        self.assertEqual(f().next(), "Import inside generator")
    ...
    def test_generator_finally(self):
        ...
        self.assertEqual(n.next(), 1)
        self.assertEqual(n.next(), 2)
        self.assertEqual(n.next(), 3)
        self.assertRaises(ZeroDivisionError, n.next)
        ...
        self.assertEqual(n.next(), 1)
        self.assertEqual(n.next(), 2)
        self.assertRaises(AssertionError, n.next)
...
""" % (args, size)
            ...
            args = range(size)
            exec "assertEqual(list(fetest(%s)),%s)" % (str(args)[1:-1], str([x*x for x in args])) in d, d
    ...
    def test_iterate_closed(self):
        ...
        g=f(l)
        self.assertTrue(g.next() == 'b')
        self.assertTrue(l == [1,0])
        self.assertRaises(MyError, g.next)
    ...
    def test_generator_attrs(self):
        ...
        temp_gen = f()
        self.assertEqual(f.func_code, temp_gen.gi_code)
    
    def test_cp24031(self):
        def f(*args):
            return args
        
        self.assertEqual(f(*(x for x in xrange(2))),
                (0, 1))
